...
export function SqliteQuerier(db: Database) {
  return async function (queryGraph: QueryGraph, schema: Schema): Promise<Either<HydraError, ResultGraph>> {
    ...
    function runQuery(query: string, params: string[]): Promise<{ [k: string]: any }[]> {
    ...
    function buildResults(rootGraph: QueryGraph, results: { [k: string]: any }[]): Either<HydraError, ResultGraph> {
      const root = rootGraph.cardinality === 'one' ?
        (results.length > 0 ? results[0].root$$id : null) :
        uniq(results.map(r => r.root$$id));

      const baseResources: { [k: string]: { [k: string]: any} } =
        uniq(flattenQueryGraph(rootGraph).map(graph => graph.type))
      ...
      const extracters = reduceQueryGraph(rootGraph, {}, (acc, graph, path) => {
        ...
        const extractFn = (row, curVal) => {
          const attrExtracters = Object.keys(rDef.attributes).reduce((acc, attr, idx) => ({
            ...acc, [attr]: row[`${table}$$${idx}`]
          }), {});
      ...
      const tables = reduceQueryGraph<{ table: string, type: string }[]>(rootGraph, [], (acc, g, path) =>
        [ ...acc, { table: ['root', ...path].join('$'), type: g.type }]
      );
    ...
    function reduceQueryGraph<T>(
      ...
      const flatWithPath = (g, p) => [
        [g, p],
        ...unnest(Object.values(g.relationships).map((rel, idx) => flatWithPath(rel, [...p, idx])))
      ];
