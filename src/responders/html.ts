...
export function HtmlResponder(
  ...
  function expandResults(resultGraph: ResultGraph) {
    ...
    const go = ({ type, id }) => {
      ...
      return mapObj(resource, (attr, name) =>
        rels.includes(name) ? goCard(attr) : attr
      );
    };

    const normalize = x =>
      Array.isArray(x) ? x.map(normalize) : { type: resultGraph.type, id: x };
