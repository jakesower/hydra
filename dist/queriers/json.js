...
function JsonQuerier(schema, baseState) {
  ...
  const baseRelationships = utils_1.fillObject(
    schema_functions_1.canonicalRelationshipNames(schema),
    []
  );
  let state = {
  ...
  function merge(resource) {
    ...
    utils_1.mapObj(resource.relationships || {}, (relationships, relationshipName) => {
      ...
      const { name: relationshipKey, locality } = schema_functions_1.canonicalRelationship(
        schema,
        type,
        relationshipName
      );
  ...
  function appendRelationship() {
    ...
    const filt = symmetric
      ...
      : arrow => arrow[locality] !== id;
    const withoutExisting = state.relationships[relationshipKey].filter(filt);
    const toAdd = (Array.isArray(foreignId) ? foreignId : [foreignId]).map(f =>
      locality === 'local' ? { local: id, foreign: f } : { local: f, foreign: id }
    );
    state.relationships[relationshipKey] = [...withoutExisting, ...toAdd];
  }
  ...
  function getOne(query) {
    ...
    const relationships = utils_1.mapObj(query.relationships || {}, (options, relationshipName) =>
  ...
  function getMany(query) {
    return Object.values(
      utils_1.mapObj(state.objects[query.type], (_, id) =>
        getOne({
          type: query.type,
          id,
  ...
  function expandRelationship(query, relationshipName, options) {
    ...
    const { name: relationshipType, locality } = schema_functions_1.canonicalRelationship(
      schema,
      type,
      relationshipName
    );
    ...
    const expand = hit =>
      schema_functions_1.isSymmetricRelationship(schema, type, relationshipName)
        ? getOne({
